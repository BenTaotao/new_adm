...
layui.define(['layer', 'form', 'yzn', 'table', 'notice', 'element', 'dragsort'], function(exports) {
    ...
    var yznForm = {
        ...
        events: {
            ...
            upload_tvfile: function (elements, onUploadSuccess, onUploadError) {
                elements = typeof elements === 'undefined' ? document.body : elements;
                // 绑定文件上传组件
                if ($(elements).length > 0) {
                    layui.define('webuploader', function(exports) {
                        ...
                        $(elements).each(function() {
                            ...
                            var $multiple = $input_file.data('multiple');
                            // 允许上传的后缀
                            var $ext = GV.site.upload_tvfile_ext;
                               
                            // 图片限制大小
                            var $size = GV.site.upload_tvfile_size*1024;
                            // 文件列表
                            var $file_list = $('#file_list_' + $input_file_name);
                            // 实例化上传
                            var uploader = WebUploader.create({
        ...
        bindevent: function () {
            ...
            events.upload_tvfile('.js-upload-tvfile,.js-upload-tvfiles');
        }
    }
    yznForm.bindevent();
